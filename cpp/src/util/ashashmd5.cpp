...
QString AsHashMD5::hashData( const QByteArray& buffer )
{
    ...
    long int length = buffer.length();

    if(length <= 0) return QString::null;

    updateHash(&ctx,data,length);
    ...
    *p++ = 0x80;
    ...
    md5Transform(ctx.buffer, (quint32 *) ctx.in, true);
...
void AsHashMD5::updateHash(md5Context* ctx, const char* data, long int length)
{
    unsigned int t;
 ...
 */
void AsHashMD5::md5Transform(unsigned int buf[4], quint32 const in[16], bool bLastCall)
{
    const quint32 * data = 0;
    bool bLittleEndian = true;
    data = in;

    // check it machine is big endian, if so convert data to little endian first
    static const qint32 w = 1;
    ...
    if( !bLittleEndian )
    {
        ...
        for( int i = 0; i< 16; ++i, byte += 4 )
            reverse[i] = byte[0] + (byte[1] << 8) + (byte[2] << 16) + (byte[3] << 24);

        data = reverse;

        if( bLastCall )
        {
            // indexes 14 & 15 on last call are used for lengths and don't require reversing
            reverse[14] = in[14];
            reverse[15] = in[15];
        }
    }
