...
FlipChartController::FlipChartController()
    : m_flipChartPath(""), m_uncompressPath(""), m_pFactory(new AsFactory()), m_flipchartNode(nullptr)
{
}
...
string FlipChartController::openFlipchart(const char *flipChartPath, const char *uncompressPath)
{
    LOGDEBUG("open file Thread is %d",QThread::currentThreadId());
    m_flipChartPath = QString(flipChartPath);

    QFileInfo fileInfo(m_flipChartPath);
    QString baseName = fileInfo.baseName();
    LOGDEBUG("baseName:%s", baseName.toUtf8().data());

    m_uncompressPath = QString(uncompressPath)+"/"+baseName;
  ...
  //  dispose();
    ...
    m_pTempVirtualFolder = m_pFactory->asGetVirtualFolder( AsFactory::AS_ID_VIRTUALRESOURCEFOLDER, virtualfolderPath );
...
void FlipChartController::asReportFileDelegate(const QString &fileName, const QByteArray &fileData)
{
...
std::string FlipChartController::asLoadOpenFlipchart(const QString &flipPath, bool recover, const QString &orgFilePathAndName)
{
    ...
    for (auto i = 0; i < pageCount; i++)
    {
        ...
        pageInfoObject["thumbnailUrl"] = (flipPath.toUtf8()+"/"+QString::number(id)+".thm").toStdString();
